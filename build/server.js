...
if (!_env.success) {
  const err = "Vari\xE1veis de ambiente incorretas";
...
// src/routes/middlewares/check-session-id-exists.ts
...
var transactions = async (app2) => {
  ...
  app2.get(
    ...
    { preHandler: [checkSessionIdExists] },
    async (request) => {
      const schema = import_zod2.z.object({ id: import_zod2.z.string().uuid() });
...
// src/server.ts
app.listen({ port: env.HOST_PORT }).then(() => console.log("Servidor executando!"));
